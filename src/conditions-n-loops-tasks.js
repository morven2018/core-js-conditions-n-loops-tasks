...
function convertToRomanNumerals(num) {
  ...
  while (curNum > 0) {
    ...
    if (curNum >= 10) {
      const delim = Math.floor(curNum / 10);
      for (let i = 0; i < delim; i += 1) {
        cur += 'X';
      }
      result += cur;
      curNum %= 10;
...
function isContainNumber(num, digit) {
  let curNum = num;
  let reminder = curNum % 10;
  while (curNum > 10) {
    if (reminder === digit) return true;
    reminder = curNum % 10;
    curNum = Math.floor(curNum / 10);
  }
  return curNum % 10 === digit || reminder === digit;
}
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < n; i += 1) {
    currmatr[i] = [];
    for (let j = 0; j < n; j += 1) {
      currmatr[i][j] = rotate[n - j - 1][i];
    }
  }
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < n; j += 1) {
      rotate[i][j] = currmatr[i][j];
    }
  }
  return rotate;
}
